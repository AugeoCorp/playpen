...
export default defineCommand({
  ...
  subCommands: {
    build: defineCommand({
      meta: { name: "build", description: "Bake the base image that sandboxes clone from" },
      args: {
        force: {
          type: "boolean",
      ...
      async run({ args }) {
        if (!args.force) {
          ...
          if (existing) {
            ...
            console.log(`bake a fresh one onto current packages: playpen image build --force`);
            return;
          }
        }
        console.error(`baking the base image; this runs once and takes a few minutes`);

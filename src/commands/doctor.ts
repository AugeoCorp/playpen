...
function checkNode(): Check {
  ...
  return stripping
    ? { status: "ok", label: "node", detail: `${process.version} runs .ts directly` }
    : {
        status: "fail",
        label: "node",
...
async function checkLimactl(): Promise<Check> {
  ...
  if (!path) {
    return { status: "fail", label: "limactl", detail: "not on PATH — install Lima" };
  }
  const { code, stdout } = await capture("limactl", ["--version"]);
  const version = code === 0 ? stdout.trim().replace(/^limactl version\s*/, "") : "unknown";
...
async function checkLimaHomeFs(): Promise<Check> {
  const home = limaHome();
  const { code, stdout } = await capture("findmnt", ["-no", "FSTYPE", "--target", home]);
  if (code !== 0) {
    return { status: "warn", label: "lima home", detail: `${home} (filesystem unknown)` };
  }
  ...
  const nodatacow = attrs.code === 0 && (attrs.stdout.split(/\s+/)[0] ?? "").includes("C");
  return nodatacow
    ? { status: "ok", label: "lima home", detail: `${home} on btrfs with nodatacow (C)` }
    : {
        status: "warn",
        label: "lima home",
        detail:
...
async function checkInstances(): Promise<Check> {
  ...
  try {
    ...
    return { status: "warn", label: "instances", detail: "could not list instances" };
  }
...
export default defineCommand({
  meta: { name: "doctor", description: "Check that the host can run playpen sandboxes" },
  async run() {
    const checks: Check[] = [
      checkNode(),
      await checkLimactl(),

...
export async function list(): Promise<Instance[]> {
  const { stdout } = mustSucceed("limactl list", await capture(LIMACTL, ["list", "--format", "json"]));
  return stdout
    .split("\n")
...
export async function createAndStart(name: string, templatePath: string): Promise<void> {
  // --tty=false: limactl otherwise prompts to edit the template.
  const code = await attach(LIMACTL, ["start", `--name=${name}`, "--tty=false", templatePath]);

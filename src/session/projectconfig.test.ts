...
import { LEGACY_IGNORE_FILE, loadProjectConfig, validateMasks } from "./projectconfig.ts";
 ...
 */
const canImportTs = Boolean((process.features as { typescript?: unknown }).typescript);
...
test("strips trailing slashes and dot segments", () => {
  assert.deepEqual(validateMasks(["target/", "./dist"]).masked, ["target", "dist"]);
});
...
test("reads masked from a default export", async (t) => {
  const dir = await project(t, {
    "playpen.config.js": 'export default { masked: ["node_modules", "target"] };',
  });
...
test("validates entries coming out of the config", async (t) => {
  const dir = await project(t, {
    "playpen.config.js": 'export default { masked: ["node_modules", "../etc"] };',
  });
  const r = await loadProjectConfig(dir);
  assert.deepEqual(r.masked, ["node_modules"]);
...
test("reports a missing default export by the file's own name", async (t) => {
  const dir = await project(t, { "playpen.config.js": 'export const masked = ["node_modules"];' });
  const r = await loadProjectConfig(dir);
...
test("reports a non-array masked instead of throwing", async (t) => {
  const dir = await project(t, { "playpen.config.js": 'export default { masked: "node_modules" };' });
  const r = await loadProjectConfig(dir);
  assert.match(r.error ?? "", /must be an array/);
});

test("a config that throws is reported, not propagated", async (t) => {
  const dir = await project(t, { "playpen.config.js": 'throw new Error("boom");' });
  const r = await loadProjectConfig(dir);
  assert.match(r.error ?? "", /boom/);
  assert.deepEqual(r.masked, []);
});

test("prefers playpen.config.ts when both files exist", { skip: !canImportTs }, async (t) => {
  const dir = await project(t, {
...
test("loads a TypeScript config", { skip: !canImportTs }, async (t) => {
  const dir = await project(t, {
    "playpen.config.ts": 'export default { masked: ["node_modules"] satisfies string[] };',
  });
  const r = await loadProjectConfig(dir);
  assert.deepEqual(r.masked, ["node_modules"]);
});

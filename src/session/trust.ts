...
export function decideTrust(pinned: string | null, current: string | null): TrustState {
...
async function readRecord(sandbox: string): Promise<TrustRecord | null> {
  try {
    return JSON.parse(await readFile(recordPath(sandbox), "utf8")) as TrustRecord;
  } catch {
    return null;
  }
}

export async function pinConfig(sandbox: string, graph: ConfigGraph): Promise<void> {
  ...
  await writeFile(recordPath(sandbox), JSON.stringify(record, null, 2) + "\n", "utf8");
}
...
function preview(graph: ConfigGraph, approved: Record<string, string> | undefined): string {
  const out: string[] = [];
  for (const f of graph.files) {
    const before = approved?.[f.rel];
    const status = before === undefined ? "new" : before === f.hash ? "unchanged" : "changed";
 ...
 */
export async function loadTrustedConfig(projectDir: string, sandbox: string): Promise<TrustedConfig> {
  ...
  try {
    ...
    return skipped("unpinned", err instanceof Error ? err.message : String(err));
  }

  const record = await readRecord(sandbox);
  const pinned = record && record.file === name && record.files ? record.hash : null;
  ...
  if (state !== "trusted") {
    ...
    console.error(`  playpen imports this file on the host, so it runs as you.`);
    console.error(`  a process inside the sandbox can write it — read it before approving.`);
    console.error(`  ${count === 1 ? "1 file" : `${count} files`} will be executed:`);
    console.error("");

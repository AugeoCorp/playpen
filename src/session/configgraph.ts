 ...
 */
function resolveSpecifier(specifier: string, fromFile: string, fromRel: string): string | null {
  ...
  throw new UnpinnableImport(fromRel, specifier, "a package; only project files can be pinned");
}

export async function readConfigGraph(projectDir: string, entryName: string): Promise<ConfigGraph> {
  ...
  while (queue.length > 0) {
    ...
    try {
      ...
      if (abs === resolve(root, entryName)) throw new Error(`${entryName} is not readable`);
      continue;
    }
    ...
    for (const spec of findSpecifiers(contents)) {
      ...
      if (targetRel.startsWith("..") || isAbsolute(targetRel)) {
        throw new UnpinnableImport(rel, spec, "which is outside the project and cannot be pinned");
      }
      queue.push(target);
    }
  }

  const sorted = [...files.values()].sort((a, b) => (a.rel < b.rel ? -1 : a.rel > b.rel ? 1 : 0));

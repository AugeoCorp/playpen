...
function selected(opts: PushOptions): SyncEntry[] {
  return SYNC_SET.filter((e) => (e.secret ? opts.includeCredentials : opts.includeConfig));
}
 ...
 */
async function fingerprint(paths: string[], opts: PushOptions): Promise<string> {
 ...
 */
const CREDENTIAL_SETTINGS = ["env", "apiKeyHelper", "awsAuthRefresh", "awsCredentialExport"];

export function filterSettings(raw: string, includeCredentials: boolean): string {
 ...
 */
export function filterClaudeJson(raw: string, projectDir: string, includeCredentials = true): string {
  const parsed = JSON.parse(raw) as Record<string, unknown>;
  const kept: Record<string, unknown> = {};

  const keys = includeCredentials ? [...IDENTITY_KEYS, ...KEEP_KEYS] : KEEP_KEYS;
...
async function pushFile(instance: string, rel: string, contents: string): Promise<string | null> {
  const script = [
    "set -eu",
    "umask 077",
  ...
  const result = await feed("limactl", ["shell", instance, "sh", "-c", script], contents);
  return result.code === 0 ? null : result.stderr.trim() || `limactl shell exited ${result.code}`;
}
 ...
 */
export async function pushClaudeConfig(instance: string, opts: PushOptions): Promise<PushResult> {
  ...
  const wantClaudeJson =
    (opts.includeConfig || opts.includeCredentials) && (await exists(claudeJsonPath));

  if (present.length === 0 && !wantSettings && !wantClaudeJson) {
    return { pushed: [], missing, bytes: 0, hash: "", skipped: false, error: "nothing to copy" };
  }
  ...
  const fail = (error: string, pushed: string[] = [], bytes = 0): PushResult => ({
    pushed,
    missing,
    bytes,
    hash,
  ...
  if (present.length > 0) {
    const tar = await captureBuffer("tar", ["-chf", "-", "-C", claudeDir(), ...present]);
    if (tar.code !== 0) return fail(tar.stderr.trim() || `tar exited ${tar.code}`);
    bytes = tar.stdout.length;

    const script = ["set -eu", "umask 077", 'mkdir -p "$HOME/.claude"', 'tar -xf - -C "$HOME/.claude"'].join("\n");
    const result = await feed("limactl", ["shell", instance, "sh", "-c", script], tar.stdout);
    if (result.code !== 0) {
      return fail(result.stderr.trim() || `limactl shell exited ${result.code}`, [], bytes);
    }
    pushed.push(...present);
  }

  if (wantSettings) {
    const err = await pushRewritten(instance, settingsPath, `.claude/${SETTINGS}`, (raw) =>
      filterSettings(raw, opts.includeCredentials),
    );
  ...
  if (wantClaudeJson) {
    const err = await pushRewritten(instance, claudeJsonPath, CLAUDE_JSON, (raw) =>
      filterClaudeJson(raw, opts.projectDir, opts.includeCredentials),
    );
    if (err) {
      return fail(`${CLAUDE_JSON} not copied (${err}) — the guest will ask you to log in`, pushed, bytes);
    }
